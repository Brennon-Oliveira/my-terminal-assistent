...
export default class Git {
    ...
    async pull(args){
        ...
        if(pullResponse.stderr){
            ...
            if(stderr.includes("You have unstaged changes")){
                ...
                `, false)
                let howProgress = await question("Como deseja prosseguir:", ["Git stash", "Git commit", "Sair"], true)
    ...
    async commit(args){
        let {finalArgs, flags} = getFlags(args, ["a", "p", "u"], ["b","t","m"])
        let settings = await getSettings();

        let branchs = await exec("git branch");
        let branch = branchs.stdout.substring(branchs.stdout.indexOf("*"))?.split(" ")[1]?.trim() || "master";
        ...
        if(commit.stderr){
            ...
            } else if(commit.stdout.includes("nenhuma modificação adicionada à submissão")){
                warning(`
                ...
                flags:[
                    {
                        ...
                        description: "Cria uma nova branch para a realizar o commit"
                    },
                    {
                        name: "-t {tipo}",
                        description: "Define o tipo do commit. Padrões conhecios: bugfix (fix), feature (feat), hotfix, release, etc"
                    },
                    {
                    ...
                    {
                        name: "-a",
                        description: 'Realiza "git pull add ." antes de realizar o commit'
                    },
                    {
                        name: "-p",
                        description: 'Realiza "git push" após de realizar o commit'
                    },
                    {
                        name: "-u",
                        description: 'Realiza "git pull" antes de realizar o commit'
                    },
                ]
            }
        ])
